...
int main( void )
{
	...
	printf( "eth1: Auto-negotiation...\n" );
	while ( !lan_autoneg( eth1 ));

	printf( "eth2: Auto-negotiation...\n" );
	...
	while ( 1 ) {
		...
		if ( timer1_flag ) {  /* ... every 10 msec */
			...
			if (( ++showtime % 1000 ) == 0 ) {  /* ... every 10 sec */
				...
				if ( lost[ eth1 ] || lost[ eth2 ]) {
					printf( "LOST (within 10 sec) eth1 = %lu, eth2 = %lu\n", lost[ eth1 ], lost[ eth2 ]);
					lost[ eth1 ] = 0;
					lost[ eth2 ] = 0;
				}
			}
...
void udp_handle_data( int ifc, uint16_t s_port, uint16_t t_port, ip_addr_t s_ip, uint8_t *data, uint16_t sz )
{
	...
	if ( next_packno[ ifc ]) {
		if ( packno != next_packno[ ifc ]) {
			if ( packno > next_packno[ ifc ]) {
				lost[ ifc ] += abs( packno - next_packno[ ifc ]);
			} else {
				++err_count[ ifc ];
				printf( "%s: TEST ERROR: Wrong packet no. (%lu).\n", ETHS[ ifc ], packno );
			}
		}
	}
