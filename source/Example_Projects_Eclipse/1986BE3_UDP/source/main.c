...
static uint32_t tx_count[ eth__COUNT_ ] = { 0, 0 };
static uint32_t rx_count[ eth__COUNT_ ] = { 0, 0 };
static uint32_t err_count[ eth__COUNT_ ] = { 0, 0 };
static uint32_t err_total = 0;
static uint32_t next_packno[ eth__COUNT_ ] = { 0, 0 };

#define PAYLOAD_SIZE  1024

uint32_t payload[ eth__COUNT_ ][ PAYLOAD_SIZE / sizeof( uint32_t )] = {{
...
int main( void )
{
	...
	lan_init( eth1, ( mac_addr_t ) MAC_ADDR_1, IP_ADDR_1, SUBNET_MASK_1 );
	lan_init( eth2, ( mac_addr_t ) MAC_ADDR_2, IP_ADDR_2, SUBNET_MASK_2 );

	printf( "lan_autoneg( eth1 )\n" );
	while ( !lan_autoneg( eth1 ));

	printf( "lan_autoneg( eth2 )\n" );
	while ( !lan_autoneg( eth2 ));
	printf( "ok\n" );
...
void udp_handle_data( int ifc, uint16_t s_port, uint16_t t_port, ip_addr_t s_ip, uint8_t *data, uint16_t sz )
{
	...
	if ( s_ip != IP_ADDR[ s_ifc ]) {
		++err_count[ ifc ];
		printf( "%s: TEST ERROR: Wrong sender IP address.\n", ETHS[ ifc ]);
	}
	/* Check packet number */
	memcpy( &packno, data, sizeof( uint32_t ));
	if ( next_packno[ ifc ]) {
		if ( packno != next_packno[ ifc ]) {
			++err_count[ ifc ];
			printf( "%s: TEST ERROR: Waited for packet no. %u, but %u received.\n", ETHS[ ifc ]
			, next_packno[ ifc ], packno );
		}
	}
	next_packno[ ifc ] = packno + 1;
	/* Check payload content */
	if ( memcmp( data + sizeof( uint32_t ), payload[ s_ifc ] + 1, sz - sizeof( uint32_t )) == 0 ) {
		++rx_count[ ifc ];
	} else {
		++err_count[ ifc ];
		printf( "%s: TEST ERROR: Wrong payload.\n", ETHS[ ifc ]);
	}
}
